...
var PID = function(Input, Setpoint, Kp,  Ki, Kd, ControllerDirection, WindowSize) {
 // this.myOutput = Output;
  this.input = Input;
  this.windowSize = WindowSize;
  this.mySetpoint = Setpoint;
	this.inAuto = false;		//default output limit corresponds to //the arduino pwm limits
...
//  this.windowSize = 2000;
  this.setOutputLimits(0, this.windowSize); 
  this.windowStartTime = this.millis();
  this.timeProportionalOutput = false;
}
...
PID.prototype.compute = function() {
  ...
  this.timeProportional(output);
}

PID.prototype.timeProportional = function(pidOutput) {
  var now = this.millis();
  if(now - this.windowStartTime>this.windowSize) { //time to shift the Relay Window
    this.windowStartTime += this.windowSize;
  }
  if((pidOutput > 100) && (pidOutput > now - this.windowStartTime)) this.timeProportionalOutput = true;
  else this.timeProportionalOutput = false;
}
